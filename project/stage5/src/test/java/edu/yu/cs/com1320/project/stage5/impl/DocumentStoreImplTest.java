...
or returned in any form as the result of any “get” or “search” request, or an action on it is undone via any call to either of the DocumentStore.undo methods.
    ...
    public void stage4TestMaxBytesWhenDoubleMaxViaPut() throws IOException {
        ...
        //uri1 and uri2 should both be gone, having been pushed out by 3 and 4
        System.out.println();
        assertNotNull(store.getDocument(this.uri3),"uri3 should still be in memory"); //3+2
        assertNotNull(store.getDocument(this.uri4),"uri4 should still be in memory"); //4+3
        assertNotNull(store.getDocument(this.uri1),"uri1 should still be in memory"); //1+4
        assertNotNull(store.getDocument(this.uri2),"uri2 should still be in memory"); //2+1
        //limit 69
        //1: 47, 2: 22, 3: 24, 4: 24
//        System.out.println(bytes1+" "+bytes2+" "+bytes3+" "+bytes4);
